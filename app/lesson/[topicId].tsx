...
function OrderQuestion({ question, onComplete, revealed }: OrderQuestionProps) {
    ...
    <View style={styles.answerArea}>
      ...
      {items.map((item, idx) => (
        ...
        >
          ...
          <Text
            style={[
              styles.orderLineText,
              dragging === idx && styles.orderLineTextDragging,
            ]}
          >
            {item.text}
          </Text>
...
function ResultScreen({ score, total, xp, onComplete }: ResultScreenProps) {
  const percentage = Math.round((score / total) * 100);
  const stars = percentage === 100 ? 3 : percentage >= 60 ? 2 : percentage >= 40 ? 1 : 0;
  ...
  return (
    <View style={styles.resultScreen}>
      <Text style={styles.resultTrophy}>{getTrophy()}</Text>
...
export default function LessonScreen() {
  ...
  }, [lessons, topicId, userStore.topicsProgress]);

  if (!topicId || !currentLesson || !currentQuestion) {
    return (
  ...
  const handleCheck = () => {
    ...
    setTimeout(() => advance(isCorrect), 1300);
  };
  ...
  const progressPercentage = finished ? 100 : Math.round(((currentQuestionIndex + 1) / totalQuestions) * 100);
        ...
        <View style={styles.answerArea}>
          ...
          {!revealed && (
            <Pressable
              style={[
                styles.checkButton,
                selected !== null ? styles.checkButtonActive : styles.checkButtonDisabled,
              ]}
              onPress={handleCheck}
            ...
            >
              <Text
                style={[
                  ...
                  selected !== null ? styles.checkButtonTextActive : styles.checkButtonTextDisabled,
                ]}
              >
                Проверить
              </Text>
      ...
      <ScrollView style={styles.container} contentContainerStyle={{ flexGrow: 1 }}>
        <View style={styles.contentContainer}>
          ...
          <View style={styles.topicBadge}>
            ...
            <Text style={styles.topicBadgeLevel}>Уровень {currentLessonIndex + 1}</Text>
          </View>
